...
- Add rate limiting on app-owned endpoints (webhook handlers, admin proxy routes) to prevent abuse
- Shopify requires HTTPS for all app URLs — never serve the app over plain HTTP, even in staging
...
```json
{
  ...
  "PreToolUse": [
    ...
    {
      "matcher": "Write|Edit",
      "command": "case \"$CLAUDE_FILE_PATHS\" in dist/*|build/*) echo 'BLOCKED: Do not edit build output — these files are generated by the build process.' && exit 2;; esac"
    }
  ]
}
```
