...
- Configure Content Security Policy (CSP) headers in `next.config.js` — restrict `script-src`, `style-src`, and `connect-src` to trusted origins
- When rendering Sanity portable text, sanitize any `dangerouslySetInnerHTML` usage — portable text renderers should handle this, but verify custom serializers don't introduce XSS
- React JSX auto-escapes interpolated values by default — avoid bypassing this with `dangerouslySetInnerHTML` unless necessary
...
```json
{
  ...
  "PreToolUse": [
    ...
    {
      "matcher": "Write|Edit",
      "command": "case \"$CLAUDE_FILE_PATHS\" in .next/*|dist/*|build/*|out/*) echo 'BLOCKED: Do not edit build output — these files are generated by the build process.' && exit 2;; esac"
    }
  ]
}
```
